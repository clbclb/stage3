...
void Client::mouseReleaseEvent(QMouseEvent *ev)
{
    ...
    else if(hb+350<pt.x()&&pt.x()<hb+450)xx=3;
    else if(hb+450<pt.x()&&pt.x()<hb+550)xx=4;
    else if(hb+550<pt.x()&&pt.x()<hb+650)xx=5;
    else return;
    if(vb-50<pt.y()&&pt.x()<vb+50)yy=0;
    else if(vb+50<pt.y()&&pt.y()<vb+150)yy=1;
    else if(vb+150<pt.y()&&pt.y()<vb+250)yy=2;
    else if(vb+350<pt.y()&&pt.y()<vb+450)yy=3;
    else if(vb+450<pt.y()&&pt.y()<vb+550)yy=4;
    else if(vb+550<pt.y()&&pt.y()<vb+650)yy=5;
    else return;
    double dx=pt.x()-(double)(hb+50+xx*100);
    double dy=pt.y()-(double)(vb+50+yy*100);
...
void Client::sendmove(int frx,int fry,int tx,int ty){
    if(isblack){
        socket->send(NetworkData(OPCODE::MOVE_OP,"A",pos_to_data(frx,fry),pos_to_data(tx,ty)));
    }
    else{
        socket->send(NetworkData(OPCODE::MOVE_OP,"A",pos_to_data(frx,fry),pos_to_data(tx,ty)));
    }
...
bool Client::judgemove(int frx,int fry,int tx,int ty){
    ...
    if(color[tx][ty]==PieceColor::NONE){
        return abs(frx-tx)+abs(fry-ty)<=1;
    }

    if(check(frx,fry,0,1,tx,ty,0)||check(frx,fry,1,1,tx,ty,0)||check(frx,fry,2,1,tx,ty,0)||check(frx,fry,3,1,tx,ty,0)){
        return 1;
    }
    return 0;
}

bool Client::check(int x,int y,int dir,int dep,int edx,int edy,bool bk){
    if(x==edx&&y==edy)return bk;
    if(dep!=1&&color[x][y]!=PieceColor::NONE)return 0;
    int xx=x+dx[dir],yy=y+dy[dir];
    if(0<=xx&&xx<=5&&0<=yy&&yy<=5)return check(xx,yy,dir,dep+1,edx,edy,bk);
    if((x==0||x==5)&&(y==0||y==5))return 0;
    if(xx<0){
        if(0<=yy&&yy<3)return check(y,x,1,dep+1,edx,edy,1);
        else return check(5-y,5,0,dep+1,edx,edy,1);
    }
    if(xx>5){
        if(0<=yy&&yy<3)return check(5-y,0,1,dep+1,edx,edy,1);
        else return check(y,5,0,dep+1,edx,edy,1);
    }
    if(yy<0){
        if(0<=xx&&xx<3)return check(0,x,3,dep+1,edx,edy,1);
        else return check(5,5-x,2,dep+1,edx,edy,1);
    }
    if(yy>5){
        if(0<=xx&&xx<3)return check(0,5-x,3,dep+1,edx,edy,1);
        else return check(5,x,2,dep+1,edx,edy,1);
    }
    return 0;
}
